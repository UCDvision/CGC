...
import resnet_maxpool as resnet_max
...
def update_resnetmax(model, debug=False):
    """
    Update a ResNet model to use :class:`EltwiseSum` for the skip
    connection.
    ...
    Returns:
        ...
        :class:`torch.models.resnet.Bottleneck` are modified.
    """
    assert isinstance(model, resnet_max.ResNet)
    ...
    for module_name, module in model.named_modules():
        if isinstance(module, resnet_max.Bottleneck):
            module.skip = EltwiseSum()
            module.forward = bottleneck_forward.__get__(module)
        elif isinstance(module, resnet_max.BasicBlock):
            module.skip = EltwiseSum()
            module.forward = basicblock_forward.__get__(module)
        else:
            continue
    ...
    def __enter__(self):
        ...
        self.patches = [
            ...
            self._patch_pool('torch.nn.functional.adaptive_avg_pool3d',
                             self.enable, self.debug),
            self._patch_pool('torch.nn.functional.max_pool1d',
                             self.enable, self.debug),
            self._patch_pool('torch.nn.functional.max_pool2d',
                             self.enable, self.debug),
            self._patch_pool('torch.nn.functional.max_pool3d',
                             self.enable, self.debug),
            self._patch_pool('torch.nn.functional.adaptive_max_pool1d',
                             self.enable, self.debug),
            self._patch_pool('torch.nn.functional.adaptive_max_pool2d',
                             self.enable, self.debug),
            self._patch_pool('torch.nn.functional.adaptive_max_pool3d',
                             self.enable, self.debug),
...
def contrastive_excitation_backprop(model,
    ...
    with ExcitationBackpropContext():
        ...
        output = model(input)
        # output = model(input, vanilla=True)
