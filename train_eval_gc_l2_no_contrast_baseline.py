...
def train(train_loader, model, contrastive_criterion, xent_criterion, optimizer, epoch, args, logger):
    ...
    gc_losses = AverageMeter('GC L2 Loss', ':.4e')
    ...
    for i in range(train_len):
        ...
        aug_output, xe_loss, sim_loss = model(images, contrastive_criterion, xe_images=xe_images, aug_images=aug_images, 
                                                      aug_params_dict=aug_params_dict, targets=targets, xent_criterion=xent_criterion, disable_contrast=True)
        xe_loss = xe_loss.mean()
        sim_loss = sim_loss.mean()

        loss = xe_loss + args.lambda_val * sim_loss
        ...
        gc_losses.update(sim_loss.item(), images.size(0))
